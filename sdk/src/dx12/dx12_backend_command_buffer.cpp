...
namespace d3d12
{
	...
	namespace command_buffer
	{
		...
		{
			// Cast opaque structures
			DX12CommandBuffer* dx12_cmdB = safe_convert<DX12CommandBuffer>(commandBuffer);
			DX12GraphicsBuffer* dx12_inputBuffer = safe_convert<DX12GraphicsBuffer>(targetBuffer);
			...
			if (dx12_inputBuffer->state == D3D12_RESOURCE_STATE_UNORDERED_ACCESS || dx12_inputBuffer->state == D3D12_RESOURCE_STATE_RAYTRACING_ACCELERATION_STRUCTURE)
			{
				...
				dx12_cmdB->cmdList()->ResourceBarrier(1, &barrier);
			}
		}
		...
		void uav_barrier_texture(CommandBuffer commandBuffer, Texture texture)
		{
			// Cast opaque structures
			DX12CommandBuffer* dx12_cmdB = safe_convert<DX12CommandBuffer>(commandBuffer);
			DX12Texture* dx12_tex = safe_convert<DX12Texture>(texture);
		...
		{
			DX12RenderTexture* dx12_rTex = safe_convert<DX12RenderTexture>(renderTexture);
		...
		void transition_to_common(CommandBuffer commandBuffer, GraphicsBuffer targetBuffer)
		{
			// Cast opaque structures
			DX12CommandBuffer* dx12_cmdB = safe_convert<DX12CommandBuffer>(commandBuffer);
			DX12GraphicsBuffer* dx12_gb = safe_convert<DX12GraphicsBuffer>(targetBuffer);
		...
		void transition_to_present(CommandBuffer commandBuffer, RenderTexture renderTexture)
		{
			// Cast opaque structures
			DX12CommandBuffer* dx12_cmdB = safe_convert<DX12CommandBuffer>(commandBuffer);
			DX12RenderTexture* dx12_renderTexture = safe_convert<DX12RenderTexture>(renderTexture);

			// Make sure the state is the right one
			direct_change_resource_state(dx12_cmdB, dx12_renderTexture->texture.resource, dx12_renderTexture->texture.state, D3D12_RESOURCE_STATE_PRESENT);
		}
		...
		CommandBuffer create_command_buffer(GraphicsDevice graphicsDevice, CommandBufferType commandBufferType)
		{
			...
			DX12GraphicsDevice* dx12_device = safe_convert<DX12GraphicsDevice>(graphicsDevice);
		...
		void destroy_command_buffer(CommandBuffer commandBuffer)
		{
			...
			for (uint32_t cmdIdx = 0; cmdIdx < DX12_NUM_FRAMES; ++cmdIdx)
			{
				...
				dx12_cmdB->commandList_internal[cmdIdx]->Release();

				// Release the command allocator
				dx12_cmdB->commandAllocator_internal[cmdIdx]->Release();
			}
		...
		void reset(CommandBuffer commandBuffer)
		{
			DX12CommandBuffer* dx12_cmdB = safe_convert<DX12CommandBuffer>(commandBuffer);
			dx12_cmdB->frameIdx++;
			dx12_cmdB->cmdAlloc()->Reset();
			dx12_cmdB->cmdList()->Reset(dx12_cmdB->cmdAlloc(), nullptr);
		}

		void close(CommandBuffer commandBuffer)
		{
			DX12CommandBuffer* dx12_cmdB = safe_convert<DX12CommandBuffer>(commandBuffer);
			dx12_cmdB->cmdList()->Close();
		}
		...
		void set_render_texture(CommandBuffer commandBuffer, RenderTexture renderTexture)
		{
			...
			direct_change_resource_state(dx12_cmdB, dx12_renderTexture->texture.resource, dx12_renderTexture->texture.state, D3D12_RESOURCE_STATE_RENDER_TARGET);

			// Set the render target and the current one
			dx12_cmdB->cmdList()->OMSetRenderTargets(1, &rtvHandle, FALSE, nullptr);
		}
		...
		void set_render_texture(CommandBuffer commandBuffer, RenderTexture renderTexture, RenderTexture depthTexture)
		{
			...
			DX12CommandBuffer* dx12_cmdB = safe_convert<DX12CommandBuffer>(commandBuffer);
			DX12RenderTexture* dx12_renderTexture = safe_convert<DX12RenderTexture>(renderTexture);
			DX12RenderTexture* dx12_depthTexture = safe_convert<DX12RenderTexture>(depthTexture);
			...
			// Set the render target and the current one
			dx12_cmdB->cmdList()->OMSetRenderTargets(1, &rtvHandle, FALSE, &depthvHandle);
		}
		...
		void set_render_texture(CommandBuffer commandBuffer, RenderTexture renderTexture0, RenderTexture renderTexture1, RenderTexture depthTexture)
		{
			// Cast opaque structures
			DX12CommandBuffer* dx12_cmdB = safe_convert<DX12CommandBuffer>(commandBuffer);
			DX12RenderTexture* dx12_renderTexture0 = safe_convert<DX12RenderTexture>(renderTexture0);
			DX12RenderTexture* dx12_renderTexture1 = safe_convert<DX12RenderTexture>(renderTexture1);
			DX12RenderTexture* dx12_depthTexture = safe_convert<DX12RenderTexture>(depthTexture);
			...
			// Flush the two barriers
			direct_change_resource_state(dx12_cmdB,
			...
			// Set the render target and the current one
			dx12_cmdB->cmdList()->OMSetRenderTargets(2, rtvHandle, FALSE, &depthvHandle);
		}
		...
		void set_render_texture(CommandBuffer commandBuffer, RenderTexture renderTexture0, RenderTexture renderTexture1, RenderTexture renderTexture2, RenderTexture depthTexture)
		{
			...
			DX12RenderTexture* dx12_renderTexture0 = safe_convert<DX12RenderTexture>(renderTexture0);
			DX12RenderTexture* dx12_renderTexture1 = safe_convert<DX12RenderTexture>(renderTexture1);
			DX12RenderTexture* dx12_renderTexture2 = safe_convert<DX12RenderTexture>(renderTexture2);
			DX12RenderTexture* dx12_depthTexture = safe_convert<DX12RenderTexture>(depthTexture);
			...
			// Set the render target and the current one
			dx12_cmdB->cmdList()->OMSetRenderTargets(3, rtvHandle, FALSE, &depthvHandle);
		}
		...
		void clear_render_texture(CommandBuffer commandBuffer, RenderTexture renderTexture, const float4& color)
		{
			...
			dx12_cmdB->cmdList()->ClearRenderTargetView(rtvHandle, &color.x, 0, nullptr);
		}
		...
		void clear_depth_texture(CommandBuffer commandBuffer, RenderTexture depthTexture, float value)
		{
			...
			// Make sure the state is the right one
			direct_change_resource_state(dx12_cmdB, dx12_depthTexture->texture.resource, dx12_depthTexture->texture.state, D3D12_RESOURCE_STATE_DEPTH_WRITE);

			// Clear with the color
			dx12_cmdB->cmdList()->ClearDepthStencilView(rtvHandle, D3D12_CLEAR_FLAGS::D3D12_CLEAR_FLAG_DEPTH, value, 0, 0, nullptr);
		}
		...
		void clear_depth_stencil_texture(CommandBuffer commandBuffer, RenderTexture depthTexture, float depth, uint8_t stencil)
		{
			...
			// Clear with the color
			dx12_cmdB->cmdList()->ClearDepthStencilView(rtvHandle, D3D12_CLEAR_FLAGS::D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, depth, stencil, 0, nullptr);
		}
		...
		void clear_stencil_texture(CommandBuffer commandBuffer, RenderTexture stencilTexture, uint8_t stencil)
		{
			// Grab the actual structures
			DX12CommandBuffer* dx12_cmdB = safe_convert<DX12CommandBuffer>(commandBuffer);
			DX12RenderTexture* dx12_stencilTexture = safe_convert<DX12RenderTexture>(stencilTexture);
			...
			// Make sure the state is the right one
			direct_change_resource_state(dx12_cmdB, dx12_stencilTexture->texture.resource, dx12_stencilTexture->texture.state, D3D12_RESOURCE_STATE_DEPTH_WRITE);

			// Clear with the color
			dx12_cmdB->cmdList()->ClearDepthStencilView(rtvHandle, D3D12_CLEAR_FLAGS::D3D12_CLEAR_FLAG_STENCIL, 1.0, stencil, 0, nullptr);
		}
		...
		void copy_buffer_into_texture(CommandBuffer commandBuffer, GraphicsBuffer inputBuffer, uint64_t bufferOffset, Texture outputTexture, uint32_t sliceIdx, uint32_t mipIdx)
		{
			...
			uint32_t actualheight = std::max(dx12_outputTex->height >> mipIdx, 1u);
			...
			outputResourceLoc.pResource = dx12_outputTex->resource;
			outputResourceLoc.Type = D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX;
			outputResourceLoc.SubresourceIndex = mipIdx + sliceIdx * dx12_outputTex->mipLevels;
		...
		size_t ComputeBC1RowPitch(size_t width)
		{
			// Each BC1 block is 4x4 pixels and takes 8 bytes.
			size_t numBlocks = (width + 3) / 4; // Equivalent to ceil(width / 4)
			size_t rowPitch = numBlocks * 8;    // Each block is 8 bytes

			// Align rowPitch to 256-byte boundaries (required for DX12 row-major textures)
			size_t alignedRowPitch = (rowPitch + 255) & ~255;

			return alignedRowPitch;
		}

		void copy_buffer_into_texture_mips(CommandBuffer commandBuffer, GraphicsBuffer inputBuffer, uint64_t initialOffset, uint32_t imageSize, Texture outputTexture, uint32_t sliceIdx)
		{
			// Get the internal command buffer structure
			DX12CommandBuffer* dx12_commandBuffer = (DX12CommandBuffer*)commandBuffer;
			DX12GraphicsBuffer* dx12_inputBuffer = (DX12GraphicsBuffer*)inputBuffer;
			...
			// Copy all the mips
			UINT64 bufferOffset = initialOffset;
			for (uint32_t mipIdx = 0; mipIdx < dx12_outputTex->mipLevels; ++mipIdx)
			{
				// Dimensions of the target MIP
				uint32_t actualwidth = std::max(dx12_outputTex->width >> mipIdx, 1u);
				uint32_t actualheight = std::max(dx12_outputTex->width >> mipIdx, 1u);
				...
				inputResourceLoc.Type = D3D12_TEXTURE_COPY_TYPE_PLACED_FOOTPRINT;
				inputResourceLoc.PlacedFootprint.Footprint.Format = dx12_outputTex->format;
				inputResourceLoc.PlacedFootprint.Footprint.RowPitch = format_alignment(dx12_outputTex->format) * actualwidth;
				inputResourceLoc.PlacedFootprint.Footprint.Width = actualwidth;
				inputResourceLoc.PlacedFootprint.Footprint.Height = actualheight;
				inputResourceLoc.PlacedFootprint.Footprint.Depth = 1;
				inputResourceLoc.PlacedFootprint.Offset = bufferOffset;
				...
				// Copy the resource
				dx12_commandBuffer->cmdList()->CopyTextureRegion(&outputResourceLoc, 0, 0, 0, &inputResourceLoc, nullptr);
		...
		void copy_texture_into_buffer(CommandBuffer commandBuffer, Texture inputTexture, uint32_t sliceIdx, uint32_t mipIdx, GraphicsBuffer outputBuffer, uint64_t bufferOffset)
		{
			...
			inputResourceLoc.pResource = dx12_inputTex->resource;
			inputResourceLoc.Type = D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX;
			inputResourceLoc.SubresourceIndex = sliceIdx * dx12_inputTex->mipLevels + mipIdx;

			// Actual size
			uint32_t mipWidth = dx12_inputTex->width >> mipIdx;
			uint32_t mipHeight = dx12_inputTex->height >> mipIdx;

			/*
			// Prepare footprint and buffer layout
			UINT64 requiredSize = 0;
			UINT64 rowSizesInBytes[6 * 7] = {};
			UINT numRows[6 * 7] = {};
			D3D12_PLACED_SUBRESOURCE_FOOTPRINT footprints[6 * 7] = {};
			D3D12_RESOURCE_DESC texDesc = dx12_inputTex->resource->GetDesc();
			dx12_inputTex->deviceI->device->GetCopyableFootprints(&texDesc, 0, 6 * 7, 0, footprints, numRows, rowSizesInBytes, &requiredSize);
			*/
			...
			outputResourceLoc.PlacedFootprint.Footprint.RowPitch = mipWidth * dx12_inputTex->alignment;
			outputResourceLoc.PlacedFootprint.Footprint.Width = mipWidth;
			outputResourceLoc.PlacedFootprint.Footprint.Height = mipHeight;
		...
		void copy_render_texture_into_buffer(CommandBuffer commandBuffer, RenderTexture inputTexture, uint32_t sliceIdx, GraphicsBuffer outputBuffer, uint64_t bufferOffset)
		{
			// Get the internal command buffer structure
			DX12RenderTexture* dx12_inputTex = (DX12RenderTexture*)inputTexture;
			copy_texture_into_buffer(commandBuffer, (Texture)&dx12_inputTex->texture, sliceIdx, 0, outputBuffer, bufferOffset);
		}
		...
		void copy_buffer_into_render_texture(CommandBuffer commandBuffer, GraphicsBuffer inputBuffer, uint64_t bufferOffset, Texture outputRenderTexture, uint32_t sliceIdx)
		{
			...
			copy_buffer_into_texture(commandBuffer, inputBuffer, bufferOffset, (Texture)&dx12_outputRT->texture, sliceIdx, 0);
		}

		void copy_buffer_into_texture_mips(CommandBuffer, GraphicsBuffer, uint64_t, Texture)
		{
		}
		...
		void set_compute_shader_cbuffer(CommandBuffer commandBuffer, ComputeShader computeShader, const char* name, ConstantBuffer constantBuffer)
		{
			...
			DX12CommandBuffer* dx12_commandBuffer = safe_convert<DX12CommandBuffer>(commandBuffer);
			DX12GraphicsDevice* deviceI = dx12_commandBuffer->deviceI;
			DX12ComputeShader* dx12_cs = safe_convert<DX12ComputeShader>(computeShader);
			DX12ConstantBuffer* dx12_cb = safe_convert<DX12ConstantBuffer>(constantBuffer);
			...
			// Get the binding
			DX12Binding bind;
			assert_msg(request_binding(dx12_cs->bindings, name, bind), "Unexistant binding.");
			...
			rtvHandle.ptr += (uint64_t)deviceI->descriptorSize[D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV] * bind.slot;
		...
		void set_compute_shader_buffer(CommandBuffer commandBuffer, ComputeShader computeShader, const char* name, GraphicsBuffer graphicsBuffer)
		{
			...
			DX12GraphicsDevice* deviceI = dx12_commandBuffer->deviceI;
			DX12ComputeShader* dx12_cs = safe_convert<DX12ComputeShader>(computeShader);
			DX12GraphicsBuffer* buffer = safe_convert<DX12GraphicsBuffer>(graphicsBuffer);
			...
			else
			{
				...
				bufferSRV.NumElements = (uint32_t)(buffer->bufferSize / buffer->elementSize);
		...
		void set_compute_shader_texture(CommandBuffer commandBuffer, ComputeShader computeShader, const char* name, Texture texture, uint32_t mipLevel)
		{
			// Grab all the internal structures
			DX12CommandBuffer* dx12_commandBuffer = (DX12CommandBuffer*)commandBuffer;
			DX12GraphicsDevice* deviceI = dx12_commandBuffer->deviceI;
			...
			else
			{
				...
				switch (dx12_tex->type)
				{
					...
					{
						D3D12_TEX1D_SRV tex1D;
						tex1D.MostDetailedMip = 0;
						tex1D.MipLevels = dx12_tex->mipLevels;
						tex1D.ResourceMinLODClamp = 0;
						srvDesc.Texture1D = tex1D;
		...
		void set_compute_shader_render_texture(CommandBuffer commandBuffer, ComputeShader computeShader, const char* name, RenderTexture renderTexture)
		{
			if (renderTexture == 0)
				return;
			...
			set_compute_shader_texture(commandBuffer, computeShader, name, (Texture)(&dx12_rTex->texture));
		}
		...
		void set_compute_shader_sampler(CommandBuffer commandBuffer, ComputeShader computeShader, const char* name, Sampler sampler)
		{
			// Grab all the internal structures
			DX12CommandBuffer* dx12_commandBuffer = (DX12CommandBuffer*)commandBuffer;
			DX12GraphicsDevice* dx12_device = dx12_commandBuffer->deviceI;
			DX12ComputeShader* dx12_cs = (DX12ComputeShader*)computeShader;
			DX12Sampler* dx12_sampler = (DX12Sampler*)sampler;
			...
			// Set the sampler
			const SamplerDescriptor& smplDesc = dx12_sampler->resource;
			...
			samplerDescriptor.MinLOD = smplDesc.minLOD;
			samplerDescriptor.MaxLOD = smplDesc.maxLOD;
			...
			rtvHandle.ptr += (uint64_t)dx12_device->descriptorSize[D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER] * bind.slot;

			// Set the sampler
			dx12_device->device->CreateSampler(&samplerDescriptor, rtvHandle);
		}
		...
		void dispatch(CommandBuffer commandBuffer, ComputeShader computeShader, uint32_t sizeX, uint32_t sizeY, uint32_t sizeZ)
		{
			...
			DX12ComputeShader* dx12_cs = (DX12ComputeShader*)computeShader;
			assert_msg(sizeX < 65535 && sizeY < 65535 && sizeZ < 65535, "Dispatch dimensions are too large.");
		...
		void set_graphics_pipeline_cbuffer(CommandBuffer commandBuffer, GraphicsPipeline graphicsPipeline, const char* name, ConstantBuffer constantBuffer)
		{
			...
			// Get the binding
			DX12Binding bind;
			assert_msg(request_binding(dx12_gp->bindings, name, bind), "Unexistant binding.");
		...
		void set_graphics_pipeline_buffer(CommandBuffer commandBuffer, GraphicsPipeline graphicsPipeline, const char* name, GraphicsBuffer graphicsBuffer, uint64_t bufferOffset)
		{
			// Grab all the internal structures
			DX12CommandBuffer* dx12_commandBuffer = (DX12CommandBuffer*)commandBuffer;
			DX12GraphicsDevice* deviceI = dx12_commandBuffer->deviceI;
		...
		void set_graphics_pipeline_texture(CommandBuffer commandBuffer, GraphicsPipeline graphicsPipeline, const char* name, Texture texture)
		{
			...
			DX12CommandBuffer* dx12_commandBuffer = safe_convert<DX12CommandBuffer>(commandBuffer);
			DX12GraphicsDevice* deviceI = dx12_commandBuffer->deviceI;
			DX12GraphicsPipeline* dx12_gp = safe_convert<DX12GraphicsPipeline>(graphicsPipeline);
			DX12Texture* dx12_tex = safe_convert<DX12Texture>(texture);
		...
		void set_graphics_pipeline_render_texture(CommandBuffer commandBuffer, GraphicsPipeline graphicsPipeline, const char* name, RenderTexture renderTexture)
		{
			...
			// Bind the texture
			set_graphics_pipeline_texture(commandBuffer, graphicsPipeline, name, (Texture)&dx12_rTex->texture);
		}
		...
		void set_graphics_pipeline_rtas(CommandBuffer commandBuffer, GraphicsPipeline graphicsPipeline, const char* name, TopLevelAS rtas)
		{
			...
			// Change the resource's state
			async_change_resource_state(dx12_gp->barriersData, dx12_rtas->data->resource, dx12_rtas->data->state, D3D12_RESOURCE_STATE_RAYTRACING_ACCELERATION_STRUCTURE);
		}
		...
		void draw_procedural_indirect(CommandBuffer commandBuffer, GraphicsPipeline graphicsPipeline, GraphicsBuffer indirectBuffer, uint64_t bufferOffset)
		{
			// Convert the opaque types
			DX12CommandBuffer* cmdI = (DX12CommandBuffer*)commandBuffer;
			DX12GraphicsPipeline* dx12_gp = (DX12GraphicsPipeline*)graphicsPipeline;
		...
		{
			DX12CommandBuffer* cmdI = safe_convert<DX12CommandBuffer>(commandBuffer);
		...
		{
			DX12CommandBuffer* cmdI = safe_convert<DX12CommandBuffer>(commandBuffer);
			DX12Query* query = safe_convert<DX12Query>(profilingScope);
		...
		void convert_mat_32_to_16(CommandBuffer commandBuffer, GraphicsBuffer inputMatrixBuffer, uint64_t inputOffset, GraphicsBuffer outputMatrixBuffer, uint64_t outputOffset, uint32_t width, uint32_t height, bool optimal)
		{
			...
			DX12CommandBuffer* cmdI = safe_convert<DX12CommandBuffer>(commandBuffer);
			DX12GraphicsBuffer* inputBuffer = safe_convert<DX12GraphicsBuffer>(inputMatrixBuffer);
			DX12GraphicsBuffer* outputBuffer = safe_convert<DX12GraphicsBuffer>(outputMatrixBuffer);

			// Leave if unsupported for now
			...
			D3D12_LINEAR_ALGEBRA_MATRIX_CONVERSION_INFO infoDesc =
			{
				...
				// SrcInfo
				{
					width * height * sizeof(float),																					// number of bytes of matrix in source 
					D3D12_LINEAR_ALGEBRA_DATATYPE_FLOAT32,																			// convert from float32_t
					D3D12_LINEAR_ALGEBRA_MATRIX_LAYOUT_COLUMN_MAJOR,																// convert from column major layout
					(width * sizeof(float))																							// comlun major stride without padding
				},

				// DataDesc
				{
					outputBuffer->resource->GetGPUVirtualAddress() + outputOffset,
					inputBuffer->resource->GetGPUVirtualAddress() + inputOffset
				}
			};

			// Query destSize
			cmdI->deviceI->previewDevice->GetLinearAlgebraMatrixConversionDestinationInfo(&infoDesc.DestInfo);

			// Run the conversion
			cmdI->cmdList()->ConvertLinearAlgebraMatrix(&infoDesc, 1);
		}
	}
}
