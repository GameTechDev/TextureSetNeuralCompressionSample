...
namespace d3d12
{
	...
	IDXGISwapChain4* CreateSwapChain(HWND hWnd, ID3D12CommandQueue* commandQueue, uint32_t width, uint32_t height, uint32_t bufferCount, DXGI_FORMAT format)
	{
		...
		assert_msg(CreateDXGIFactory2(createFactoryFlags, IID_PPV_ARGS(&dxgiFactory2)) == S_OK, "DXGI Factory 2 request failed.");
		...
		HRESULT res = dxgiFactory2->CreateSwapChainForHwnd(commandQueue, hWnd, &swapChainDesc, nullptr, nullptr, &swapChain1);
		assert_msg(res == S_OK, "Create Swap Chain failed.");
	...
	namespace swap_chain
	{
		...
		SwapChain create_swap_chain(RenderWindow renderWindow, GraphicsDevice graphicsDevice, CommandQueue commandQueue, TextureFormat format)
		{
			...
			for (uint32_t n = 0; n < DX12_NUM_FRAMES; n++)
			{
				...
				currentTexture.texture.mipLevels = 1;
		...
		void present(SwapChain swapChain, CommandQueue)
		{
