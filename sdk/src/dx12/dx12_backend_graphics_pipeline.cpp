...
namespace d3d12
{
    namespace graphics_pipeline
    {
        ...
        void fill_sample_desc(const GraphicsPipelineDescriptor&, DXGI_SAMPLE_DESC& sampleDesc)
        {
            sampleDesc.Count = 1;
                                                            ...
                                                            std::vector<LPCWSTR>& arguments, std::vector<DxcDefine>& definesArray)
        {
            ...
            D3D12_INPUT_ELEMENT_DESC inputElementDescs[] = {
                ...
                {"MATID", 0, DXGI_FORMAT_R32_UINT, 0, 40, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0}
            };
            ...
            DX12GraphicsPipeline* dx12_gp = new DX12GraphicsPipeline();
            dx12_gp->device = deviceI;

            // Vertex pipeline
            dx12_gp->vertBlob = vertexBlob;
            dx12_gp->hullblob = hullBlob;
            dx12_gp->domainBlob = domainBlob;
            dx12_gp->geometryBlob = geometryBlob;

            // Fragment pipeline
            dx12_gp->fragBlob = fragmentBlob;

            // Do the reflection
            uint32_t cbvCount = 0, srvCount = 0, uavCount = 0, samplerCount = 0;
            {
                query_bindings(dx12_gp->vertBlob, cbvCount, srvCount, uavCount, samplerCount, dx12_gp->bindings);
                if (dx12_gp->hullblob != nullptr)
                    query_bindings(dx12_gp->hullblob, cbvCount, srvCount, uavCount, samplerCount, dx12_gp->bindings);
                if (dx12_gp->domainBlob != nullptr)
                    query_bindings(dx12_gp->domainBlob, cbvCount, srvCount, uavCount, samplerCount, dx12_gp->bindings);
                query_bindings(dx12_gp->fragBlob, cbvCount, srvCount, uavCount, samplerCount, dx12_gp->bindings);
            }

            // Root signature
            dx12_gp->rootSignature = create_root_signature(deviceI, srvCount, uavCount, cbvCount, samplerCount);

            // PSO descriptor that we need to fill
            psoDesc.pRootSignature = dx12_gp->rootSignature->rootSignature;
            ...
            HRESULT hr = deviceI->device->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&dx12_gp->pipelineStateObject));
            ...
            assert(deviceI->device->CreateCommandSignature(&commandSignatureDesc, nullptr, IID_PPV_ARGS(&dx12_gp->commandSignature)) == S_OK);

            // Create the descriptor heap for this compute shader
            dx12_gp->CSUHeaps.push_back(create_descriptor_heap_suc(deviceI, srvCount, uavCount, cbvCount));
            dx12_gp->samplerHeaps.push_back(create_descriptor_heap_sampler(deviceI, std::max(1u, samplerCount)));
            dx12_gp->cmdBatchIndex = UINT32_MAX;
            dx12_gp->nextUsableHeap = 0;
            dx12_gp->srvCount = srvCount;
            dx12_gp->uavCount = uavCount;
            dx12_gp->cbvCount = cbvCount;
            dx12_gp->samplerCount = samplerCount;
            dx12_gp->stencilRef = gpd.depthStencilState.stencilRef;

            // Resource counting
            deviceI->allocatedGP++;

            // Convert to the opaque structure
            return (GraphicsPipeline)dx12_gp;
        }
